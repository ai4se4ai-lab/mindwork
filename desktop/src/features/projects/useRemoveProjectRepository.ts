...
export type RemoveProjectRepositoryInput = {
  project: Project;
  repository: Repository;
};

/**
 * Drops a repository from a project's `kind:30621` member list, keeping the
 * rest of the project intact. Only the project's literal signer can re-sign
 * this replaceable event — callers must confirm `identityPubkey ===
 * project.owner` before invoking this; when the project was published by an
 * agent, ask the agent instead (see `buildRemoveRepoPrompt`).
 */
async function removeProjectRepository({
  project,
  repository,
}: RemoveProjectRepositoryInput) {
  ...
  if (!liveAddresses.includes(repository.repoAddress)) {
    throw new Error("This repository is not part of the project.");
  }
...
export function useRemoveProjectRepositoryMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: removeProjectRepository,
